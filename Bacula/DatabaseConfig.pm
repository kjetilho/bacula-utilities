...
sub new {
    ...
    $p->getoptions(
        ...
        "database-host|H=s"   => \$override->{'database-host'},
...
sub config {
    ...
    # use socket (default behaviour) if on localhost and database-user
    # matches username running this process.
    my $username = getpwuid($>);
    unless ($self->{config}->{'database-host'} eq 'localhost'
            and $username and $self->{config}->{'database-user'} eq $username) {
...
sub version {
    ...
    unless (defined $self->{version}) {
        ...
        $self->{version} = $self->{dbh}->selectrow_array(
            'SELECT versionid FROM version');
    }
    $self->{version};
}

sub has_filename_table {
    my ($self) = @_;
    $self->version < 2171;
}
